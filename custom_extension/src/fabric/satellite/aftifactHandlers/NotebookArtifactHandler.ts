...
export class NotebookArtifactHandler implements IArtifactHandler {
    ...
    //         request.pathTemplate = request.pathTemplate.replace('/items/', '/notebooks/');
    //     }
    ...
    readWorkflow = {
        onBeforeRead(artifact: IArtifact, options: IApiClientRequestOptions): Promise<IApiClientRequestOptions> {
            const x = vscode.window.showInformationMessage('Reading notebook artifact');
            if (options.pathTemplate) {
                options.pathTemplate = options.pathTemplate.replace('/items/', '/notebooks/');
            }
